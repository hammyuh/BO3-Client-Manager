...
public partial class MainWindow : Window, IComponentConnector
{
	...
	private async void MainWindow_Loaded(object sender, RoutedEventArgs e)
	{
		...
		{
			MessageBox.Show("BO3 Client Manager could not fetch the executable hashes at startup and no cached hashes file exists, so the app must close. Please connect to the internet and relaunch the app.", "Hash Update Failed", MessageBoxButton.OK, MessageBoxImage.Hand);
	...
	private async void HashRetryTimer_Tick(object? sender, EventArgs e)
	{
		if (await HashUpdater.UpdateHashes())
		{
			...
			if (!hashRetryNotified)
			{
				...
				MessageBox.Show("BO3 Client Manager fetched the latest executable hashes after reconnecting to the internet.", "Hash Update Complete", MessageBoxButton.OK, MessageBoxImage.Asterisk);
				StartApp();
			}
		}
	}
